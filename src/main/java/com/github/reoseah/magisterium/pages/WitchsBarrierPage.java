...
public class WitchsBarrierPage extends MagisteriumPage {
	...
	protected static final TranslatableText DESC2 = new TranslatableText("container.magisterium.witches_barrier.desc2");
	...
	public void drawForeground(MagisteriumScreen screen, MatrixStack matrices, int mouseX, int mouseY) {
		...
		int titleX = LEFT_X_CENTER - tr.getWidth(WitchsBarrierPage.TITLE) / 2;
		tr.draw(matrices, WitchsBarrierPage.TITLE.formatted(Formatting.UNDERLINE), titleX, Y_OFFSET + 4, 0x000000);

		List<OrderedText> desc0 = tr.wrapLines(WitchsBarrierPage.DESC0, 108);
		...
		List<OrderedText> desc1 = tr.wrapLines(WitchsBarrierPage.DESC1, 108);
		for (int i = 0; i < desc1.size(); i++) {
			tr.draw(matrices, desc1.get(i), LEFT_X_OFFSET, 140 + i * 9, 0x000000);
		}

		List<OrderedText> desc2 = tr.wrapLines(WitchsBarrierPage.DESC2, 104);
	...
	public boolean canQuickTransfer(Slot slot, int index, ItemStack stack) {
		...
		return switch (index) {
		case 0 -> item == Items.REDSTONE || item == Items.GLOWSTONE_DUST || item == Items.GUNPOWDER
				|| item == Items.SUGAR;
		case 1 -> item == Items.SLIME_BALL || item == Items.MAGMA_CREAM;
		default -> false;
		};
	}
